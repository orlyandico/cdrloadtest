...
sub create_parent_tables {
    ...
    my $sth = $dbh->prepare("select table_name from information_schema.tables where table_name = '$table_name'");
    my $tab = 0;
    $sth->execute;
    $sth->bind_col(1, \$tab);
    $sth->fetch;
...
INSERT INTO $table_name(cdrType, callId, aNumber, bNumber, origCauseLocation, origCauseValue,
	...
	while ($count < $batchSize) {
		...
		my $pk1 = Digest::MD5::md5_hex($aNumber . $bNumber . $dateTimeConnect);
