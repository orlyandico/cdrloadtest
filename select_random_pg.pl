...
sub create_sql_table {
    ...
    my $sth = $d->prepare(
"select table_name from information_schema.tables where table_name like 'cdrdata_201%' order by table_name"
    );
    $sth->execute();
    ...
    while ( $sth->fetch ) {
        if ( $count == 0 ) {
            ...
            $sql .=
" union all select anumber, bnumber from $this_table where anumber='$aNum' ";
        }
        $count++;
    }
    $sth->finish;
...
while (1) {
    ...
    if ( $select_mode eq "PARTITION" ) {
        $sth = $dbh->prepare(
            "SELECT anumber, bnumber FROM cdrdata_part WHERE anumber = ?");
    }
    while ( $sub_count < 100 ) {
